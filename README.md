...
- **AI 語意判讀**（選用，需 Anthropic API key）：結果卡片的「🤖 AI 語意判讀」一鍵完成「本機去識別化 → 只送遮蔽後文字給 Claude 依 `review-guidelines.md` 準則審查 → 回覆在本機還原代號顯示」。Key 只存瀏覽器 localStorage；模型可選 Opus 4.8／Sonnet 5／Haiku 4.5，顯示 tokens 與估算成本；審查準則在打包時嵌入，更新準則後重打包即生效。使用前請確認資安政策允許將去識別化文字送至 Anthropic API。
- **去識別化匯出**（機密合約交 AI 審查用）：結果卡片上的「🔒 去識別化匯出」在**本機**把公司名、金額（阿拉伯＋國字）、統編、電話、Email、地址、帳號、單號、專案名、聯絡人替換成代號，產出兩個檔——可上傳給 AI 審查的文字檔，與**只留本機**的對照表 CSV（拿審查意見回來自行對照還原）。百分比、天數與條款文字保留（審查本體）；可加自訂遮蔽詞（專案代號、產品型號）。上傳前請自行抽查一遍，並先與資安確認去識別化文件的分級。
- **自動學習**：對每項結果按「✘ 誤報」——
